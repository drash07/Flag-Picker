...
class ReactFlagsSelect extends Component {
	...
	onSelectWithKeyboard(countryCode, evt) {
				...
				{this.state.openOptions &&
					<div ref="flagOptions" style={{fontSize: `${optionsSize}px`}} className={`flag-options ${alignClass}`}>
						...
						{(this.state.filter ? this.state.filteredCountries : this.state.countries).map( countryCode =>
							...
							<div className={`flag-option ${this.props.showOptionLabel ? 'has-label' : ''}`} key={countryCode} tabIndex="0" onClick={(evt) => this.onSelect(countryCode,evt)} onKeyUp={evt => this.onSelectWithKeyboard(countryCode, evt)}>
								<Checkbox onChange={(evt) => this.onCheckBoxChange}>
